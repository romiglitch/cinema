...
Base URL: [http://100.94.185.70:50594/](http://100.94.185.70:50594/)
Test card: ISRAEL ISRAELI | 1234567890123456 | 12/27 | 123
Test user: [kladnitsky.romi@gmail.com](mailto:kladnitsky.romi@gmail.com) / 1234
...
## TC-12 — AI chat mood buttons and custom genre request

On any page with the master layout (e.g. homepage): open the AI chat (launcher + chat window present). Click the predefined mood buttons (מצחיק, מפחיד, דייט). Then type a free-text request for other genres, e.g. "אני מחפש סרט בז'אנר דרמה או מדע בדיוני", and send. Verify: (a) each mood click adds a user message and an AI reply, (b) the custom genre message is sent, (c) the AI responds with a movie recommendation (bold `**title**`), not the "all AI services busy" fallback.
...
| TC-04 | ✅ PASS | Movies date dropdown shows screenings (available + sold-out)     |
...
| TC-10 | ✅ PASS | Redirects to Login.aspx when not authenticated                   |
| TC-11 | ✅ PASS | HTTP 200, no server errors                                       |
| TC-12 | ✅ PASS | Mood buttons + custom דרמה/מדע request return AI recommendations |


*Last run: 2026-06-10 — 12/12 passed*
